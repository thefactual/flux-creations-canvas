import { Lock, Play, Heart, Check, Image as ImageIcon } from "lucide-react";
import { LOCKED_FEED, type LockedPost } from "@/data/creator";
...
function Tile({ post }: { post: LockedPost }) {
  ...
  return (
    ...
    >
      {/* Locked overlay */}
      {!isOpen && (
        <div className="absolute inset-0 flex flex-col items-center justify-center gap-2 bg-black/55 backdrop-blur-md transition-colors group-hover:bg-black/45">
      ...
      <div className="absolute bottom-2 left-2 flex items-center gap-1 text-xs font-semibold text-white/90">
        <Heart className="h-3.5 w-3.5 fill-white/90" /> {compact(post.likes)}
    ...
    <section className="mx-auto mt-8 max-w-3xl px-4 pb-12">
      <div className="mb-3 flex items-center justify-between">
        ...
        <span className="text-xs text-white/40">
          {LOCKED_FEED.length} posts · {LOCKED_FEED.filter((p) => p.priceCents > 0).length} PPV
        </span>
      </div>
      ...
      {!isSubscribed && (
        <button
          ...
          className={cn(
            "mb-4 flex w-full items-center justify-between rounded-2xl border border-brand-500/40 bg-brand-500/[0.08] px-4 py-3 text-left transition-colors hover:bg-brand-500/[0.14]",
          )}
        >
          <span className="text-sm">
            <span className="font-bold">Subscribe to unlock everything</span>
            <span className="block text-white/55">
              All photos & videos + DM access. From $19.99/mo.
            </span>
          </span>
